# Role: improved real-capture correlation and phase-quality analysis script.
import numpy as np
...
# Plotting
DECIMATE_FULL_PLOT = 50

# Important:
# np.correlate() already conjugates the second argument for complex data.
# Correct matched-filter convention is USE_LEGACY_CORRELATION = False.
# If you want to reproduce your previous results exactly, set this to True.
...
srs_bandwidth_hz_lte = len(sc_idx) * 15e3 * cfg_ded.transmissionCombNum
lte_bandwidth_hz = total_subcarriers * 15e3

print("Total active subcarriers:", total_subcarriers)
print("SRS bandwidth using 15 kHz spacing [MHz]:", srs_bandwidth_hz_lte / 1e6)
print("LTE-like active bandwidth using 15 kHz spacing [MHz]:", lte_bandwidth_hz / 1e6)

effective_scs = sample_rate / N_FFT
print("Effective subcarrier spacing from sample_rate / N_FFT [kHz]:", effective_scs / 1e3)

if abs(effective_scs - 15e3) / 15e3 > 0.05:
    print()
    print("WARNING:")
    print("sample_rate / N_FFT is not close to 15 kHz.")
    print("This may still work if the transmitter waveform uses these settings,")
    print("but confirm the transmitter sample rate and FFT size with your supervisor.")
...
def matched_correlate(rx, template):
    """
    Matched correlation for complex IQ data.

    np.correlate(a, v) already uses conjugate(v) internally for complex data.
    Therefore, the normal matched filter is np.correlate(rx, template).

    Legacy mode uses template.conj() to reproduce the earlier implementation.
    """
...
print()
print("NOTE:")
print("The strongest RX0 and RX1 peaks may occur at different SRS periods in long captures.")
print("For AoA, use the joint detected peaks, not peak0 and peak1 separately.")
print("RX1 - RX0 strongest peak difference [samples]:", peak1 - peak0)

# ============================================================
# RAW / PRE-MAX-NORMALIZED CORRELATION QUALITY CHECK
# ============================================================
...
print("============================================================")
print("Raw/pre-max-normalized joint peak index:", raw_peak)
print("Raw/pre-max-normalized joint peak time [ms]:", raw_peak / sample_rate * 1000)
print("Joint peak-to-floor [dB]:", raw_quality_db)
...
if T_per is not None:
    ...
    print("Expected approximate number of SRS peaks:", expected_num_peaks)
...
if len(peaks) > 1:
    ...
    print()
    print("============================================================")
    print("PEAK INTERVAL STATISTICS")
    print("============================================================")
    print("Mean interval [ms]:", np.mean(intervals_ms))
    print("Std interval [ms]:", np.std(intervals_ms))
    print("Min interval [ms]:", np.min(intervals_ms))
    print("Max interval [ms]:", np.max(intervals_ms))
...
avg_phase_weighted = np.nan
phase_coherence_weighted = np.nan

if len(phase_diffs) > 0:
    avg_phase = np.angle(np.mean(np.exp(1j * phase_diffs)))
    phase_coherence = np.abs(np.mean(np.exp(1j * phase_diffs)))

    weights = np.abs(h0_values) * np.abs(h1_values)

    if np.sum(weights) > 0:
        weighted_vector = np.sum(weights * np.exp(1j * phase_diffs)) / np.sum(weights)
        avg_phase_weighted = np.angle(weighted_vector)
        phase_coherence_weighted = np.abs(weighted_vector)
    ...
    print()
    print("============================================================")
    print("WEIGHTED AVERAGE PHASE DIFFERENCE")
    print("============================================================")
    ...
    print("============================================================")
    print("Normal mean phase [deg]:", np.mean(phase_deg))
    print("Circular average phase [deg]:", np.rad2deg(avg_phase))
    print("Weighted circular average phase [deg]:", np.rad2deg(avg_phase_weighted))
...
# Uncomment later after hardware calibration and antenna spacing measurement:
#
# antenna_spacing = wavelength / 2
# calibration_phase = 0.0
# corrected_phase = np.angle(np.exp(1j * (avg_phase_weighted - calibration_phase)))
# arg = corrected_phase * wavelength / (2 * np.pi * antenna_spacing)
...
# ============================================================
# PLOT FULL CORRELATION, DECIMATED
# ============================================================

t_corr_ms = np.arange(len(corr_joint)) / sample_rate * 1000
plot_step = max(1, int(DECIMATE_FULL_PLOT))

plt.figure()
plt.plot(t_corr_ms[::plot_step], corr0_norm[::plot_step], label="RX0 correlation")
plt.plot(t_corr_ms[::plot_step], corr1_norm[::plot_step], label="RX1 correlation")
plt.plot(t_corr_ms[::plot_step], corr_joint[::plot_step], label="Joint correlation", linewidth=2)

for p in peaks:
    plt.axvline(p / sample_rate * 1000, linestyle=":", alpha=0.4)
...
if len(phase_diffs) > 0:
    ...
    plt.figure()
    plt.plot(valid_peak_times_ms, np.rad2deg(phase_diffs), marker="o", label="Per-SRS phase")
    plt.axhline(np.rad2deg(avg_phase), linestyle="--", label="Circular average")
    plt.axhline(np.rad2deg(avg_phase_weighted), linestyle=":", label="Weighted average")
...
np.savez(
    ...
    avg_phase_weighted=avg_phase_weighted,
    phase_coherence_weighted=phase_coherence_weighted,
    h0_values=h0_values,
    h1_values=h1_values,
